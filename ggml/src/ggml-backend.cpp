    ...
    if (allow) {
        for (int i = 0; i < GGML_MAX_SRC; i++) {
            ...
            if (src->buffer != NULL && src->buffer->usage == GGML_BACKEND_BUFFER_USAGE_WEIGHTS) {
                ...
                // streamed expert caches are rewritten between waves. Offloading a host cache
                // snapshots it onto the device once, and every later fill is invisible to the GEMM.
                const bool stream_cache = src->name != nullptr && strstr(src->name, "stream_cache") != nullptr;
                // check if a backend with higher prio wants to offload the op
                if (!stream_cache && sched->op_offload && src_backend_id == sched->n_backends - 1 && ggml_backend_buffer_is_host(src->buffer)) {
