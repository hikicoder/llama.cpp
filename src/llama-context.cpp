...
void llama_perf_context_print(const llama_context * ctx) {
    ...
    if (const auto * mstream = ctx->get_model().moe_stream()) {
        // No role: this path belongs to one context and therefore one streaming model, and its
        // callers (llama-bench, perplexity, imatrix, batched-bench, mtmd-cli) print exactly one
        // block. NULL reproduces the line they printed before roles existed. The label is for
        // the server, where two managers write into one log.
        mstream->print_stats(nullptr);
    }
}
