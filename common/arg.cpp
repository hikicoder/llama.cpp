...
common_params_context common_params_parser_init(common_params & params, llama_example ex, void(*print_usage)(int, char **)) {
    ...
    add_opt(common_arg(
        {"--moe-stream-l2"}, "N",
        "host-RAM tier below the VRAM expert cache for --moe-stream, in GiB (0 = off); implies "
        "--moe-stream. A miss that hits this tier uploads from pinned host memory instead of "
        "reading the drive. Measured 2026-08-09 at 32 GiB: 1.40-1.47x throughput. The memory is "
        "page-locked, so it is taken away from the rest of the machine for the life of the process",
        [](common_params & params, int value) {
            if (value < 0) {
                throw std::invalid_argument("must not be negative");
            }
            params.moe_stream = true;
            params.moe_stream_l2_gib = (uint32_t) value;
        }
